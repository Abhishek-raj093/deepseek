...
import Markdown from 'react-markdown' //Importing react-markdown to render markdown content it will Structure the  content in a better way
import Prism from 'prismjs' // Importing Prism for syntax highlighting it will help to highlight the code in the response
...
const Message = ({ role, content }) => {
    ...
    useEffect(()=>{
        Prism.highlightAll()
    }, [content]) // Prism.highlightAll() will highlight the code in the response

    const copyMessage = ()=>{
        navigator.clipboard.writeText(content) // Copying the content to clipboard
        toast.success('Message copied to clipboard') // Showing success message
    }
